	...
	<view class="sort" id="Sort">
		<view class="sortline">
			...
			<div v-show="allshow.filtrateshow">
				<filtrate-box ref="filtratebox2" @cancelFiltrate="cancelFiltrate" @toFiltra="toFiltra"></filtrate-box>
			</div>
		</view>
...
// 			}).exec() 
		...
		methods: {
			resetAllSort(){
				...
				// 重置筛选选项
				this.filtraoption = {};
				this.$refs.filtratebox2.resetCondition('all');
			},
			...
			delCondition(key) {
				...
				switch(key) {
					...
					case 'spdate':
						...
						this.$refs.filtratebox2.resetCondition(key);
						break;
					case 'leage':
