<template>
	<view class="search">
		<div class="searchbox">
			<div class="icon" @click="toCancel">
				<icon class="cancel" type="cancel" color="#999"/>
			</div>
			<div class="input">
				<i class="icon"></i>
				<input type="text" v-model="kword" placeholder="请输入关键字">
			</div>
			<span class="tosearch" @click="toSearch">搜索</span>
		</div>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			toSearch() {
				let kwd = this.kword.trim();
				
				this.$emit('toSearch', kwd);
