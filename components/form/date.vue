<template>
	<view class="selectline">
		<picker class="picker" mode="date" fields="month" :value="date" @change="dateChange">
			<div class="line">
...
<script>
	export default {
		...
		computed: {
			info() {
				return this.date ? this.date : `请选择${this.placeholder?this.placeholder:this.name}`
			},
			activeColor() {
				return this.date ? '#000000' : '#999999'
			}
		},
		methods: {
			dateChange(r) {
				console.log(r);
				this.date = r.detail.value;
				this.$emit('listChange', this.date, this.type);
			},
			compReset() {
				this.date = '';
			}
